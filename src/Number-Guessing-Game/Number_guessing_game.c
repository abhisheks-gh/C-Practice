...
int main()
{
  int num, guess, no_of_guesses = 1; /*We can declare a variable to hold the current time from the system using: 
                                       time_t now = time(0); */
  ...
  srand(time(0));
  num = rand() % 100 + 1; /*num=rand(); can generate any random number.
